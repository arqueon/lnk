#!/usr/bin/env bash
# ═══════════════════════════════════════════════════════════════════
#  launch-homelab-menu.sh — Menú dinámico de servicios para MangoWM
# ═══════════════════════════════════════════════════════════════════
...
DATA_DIR="${XDG_CONFIG_HOME:-$HOME/.config}/mango/data"
...
while IFS=$'\t' read -r col1 col2 col3 col4 col5; do
    if [[ -n "$col5" ]]; then
        ...
        # Retrocompatibilidad con TSV de 4 columnas
        icon="${col1%% *}"
        name="${col1#* }"
        url="$col2"
        category="$col3"
    ...
    # Formato con espacio holgado entre ícono y nombre: "󰽉   Digipad                  │ Docencia       │ Muros..."
    clean_display="$(printf "%s   %-24.24s │ %-14.14s │ %s" "$icon" "$name" "$category" "$desc")"
